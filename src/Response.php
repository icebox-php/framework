...
class Response {
    ...
    private $status_code;
    private $headers;
    private $protocol_version = 1.0; // '1.0', '1.1', '2.0', '2'
    private $http_status_codes = array(
      ...
      420 => "Enhance Your Calm", 422 => "Unprocessable Entity",
    ...
    private function clearFlash() {
      ...
      // var_dump($_SESSION['_ice_flash']);

      if( isset($_SESSION['_ice_flash'])  ) {
        foreach($_SESSION['_ice_flash'] as $key => $item) {
          if(isset($item['life'])) {
              if($item['life'] == 0) {
                unset($_SESSION['_ice_flash'][$key]);
              } else if($item['life'] > 0) {
                $_SESSION['_ice_flash'][$key]['life'] = 0;
              }
          }
        }
      }
    ...
    private function sendHeaders() {
      ...
      if (headers_sent()) { // do not send no more header
        return true;
      }

      foreach($this->headers as $header) {
        $replace = (isset($header[2])) ? filter_var($header[2], FILTER_VALIDATE_BOOLEAN) : false;
    ...
    private function getReasonPhrase() {
      if(isset($this->http_status_codes[$this->status_code])) {
        return $this->http_status_codes[$this->status_code];
      }
      return '';
    }
